  ...
  module ProxmoxVmCommands
    ...
    def create_vm(args = {})
      ...
      if image_id
        ...
        logger.warn(format(('create vm: args=%<args>s'), args: args))
        vm = node.send(vm_collection(type)).create(parse_typed_vm(args, type))
        start_on_boot(vm, args)
      end
    rescue StandardError => e
      logger.warn(format(('failed to create vm: %<e>s'), e: e))
    ...
    def save_vm(uuid, new_attributes)
      ...
      if templated == '1' && !vm.templated?
        ...
        logger.debug(format(('save_vm(%<vmid>s) volumes_attributes=%<volumes_attributes>s'), vmid: uuid,
