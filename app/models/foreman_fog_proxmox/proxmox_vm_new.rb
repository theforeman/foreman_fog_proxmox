...
module ForemanFogProxmox
  module ProxmoxVmNew
    ...
    def add_default_typed_interface(type, new_attr)
      ...
      logger.debug(format('add_default_typed_interface(%<type>s) to new_attr=%<new_attr>s', type: type,
new_attr: new_attr))
      new_attr
    end
    ...
    def add_default_typed_volume(new_attr)
      ...
      logger.debug(format('add_default_typed_volume(%<type>s) to new_attr=%<new_attr>s', type: type,
new_attr: new_attr))
      new_attr
    end
    ...
    def new_typed_vm(new_attr, type)
      ...
      new_attr_type ||= type
      logger.debug(format('new_typed_vm(%<type>s): new_attr_type=%<new_attr_type>s', type: type,
new_attr_type: new_attr_type))
      logger.debug(format('new_typed_vm(%<type>s): new_attr=%<new_attr>s', type: type, new_attr: new_attr))
      ...
      logger.debug(format('new_typed_vm(%<type>s): options=%<options>s', type: type, options: options))
      vm_h = parse_typed_vm(options, type).deep_symbolize_keys
      logger.debug(format('new_typed_vm(%<type>s): vm_h=%<vm_h>s', type: type, vm_h: vm_h))
